...
<div class="container mt-5">
    ...
    <table class="table">
        ...
        <button type="button" class="btn btn-warning btn-sm ml-2" data-toggle="modal" data-target="#editClientModal{{$cliente['id']}}">
    Editar
</button>
                </td>
            </tr>
...
<div class="modal fade" id="editClientModal{{$cliente['id']}}" tabindex="-1" role="dialog" aria-labelledby="editClientModalLabel{{$cliente['id']}}" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="updateForm_{{ $cliente['id'] }}" action="{{ route('formulario.update', ['cliente' => $cliente['id']]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="form-group">
