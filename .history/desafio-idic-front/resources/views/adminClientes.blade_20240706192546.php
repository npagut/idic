...
<div class="container mt-5">
    ...
    <table class="table">
        ...
        <form id="editForm_{{ $cliente['id'] }}" action="{{ route('formulario.actualizar', ['id' => $cliente['id']]) }}" method="POST" class="d-inline">
    @csrf
    @method('PUT')
    <button type="submit" class="btn btn-warning btn-sm ml-2">Editar</button>
</form>
                </td>
            </tr>
            @endforeach
...
<div class="modal fade" id="editClientModal{{$cliente['id']}}" tabindex="-1" role="dialog" aria-labelledby="editClientModalLabel{{$cliente['id']}}" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="editClientForm{{$cliente['id']}}" action="{{ route('formulario.editar', ['id' => $cliente['id']]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="form-group">
                        <label for="editDireccion{{$cliente['id']}}">Dirección</label>
                        <input type="text" class="form-control" id="editDireccion{{$cliente['id']}}" name="direccion" value="{{$cliente['direccion']}}" required>
                    </div>
                    <div class="form-group">
                        <label for="editTelefono{{$cliente['id']}}">Teléfono</label>
                        <input type="text" class="form-control" id="editTelefono{{$cliente['id']}}" name="telefono" value="{{$cliente['telefono']}}" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
